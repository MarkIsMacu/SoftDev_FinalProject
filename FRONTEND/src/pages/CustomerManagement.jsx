...
import { suggestLoginEmail, generateTempPassword, isEmailTaken } from '../store/accounts.js';
...
const CustomerManagement = () => {
  const { customers, tickets, registerCustomer } = useData();
  const [search, setSearch] = useState('');
  const [modal, setModal] = useState(null);
  const [form, setForm] = useState(EMPTY_FORM);
  const [formErr, setFormErr] = useState('');
  // After registration, show credentials
  const [credentials, setCredentials] = useState(null); // { loginEmail, tempPassword, name }
  ...
  // Auto-suggest login email when name changes
  ...
  const handleRegister = () => {
    ...
    if (isEmailTaken(form.loginEmail)) { setFormErr(`"${form.loginEmail}" is already taken. Try another login email.`); return; }
      ...
      <motion.div variants={container} initial="hidden" animate="show">
        <motion.div variants={row} className="page-header">
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            <motion.button className="btn btn-ghost" whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }}><RefreshCw size={16} /></motion.button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ padding: '0.75rem 1rem', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-1)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  ...
                  {/* Full Name — letters and spaces only, max 80 chars */}
                  <div>
                    ...
                    <input className="input" placeholder="e.g. Juan dela Cruz"
                      ...
                      value={form.name}
                      onChange={e => {
                        // Only allow letters, spaces, hyphens, and periods (for names like "del, Jr., O'Brien")
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                    {/* Phone — digits, +, -, spaces only, max 15 chars */}
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ padding: '0.75rem 1rem', borderRadius: 10, background: 'rgba(0,229,255,0.04)', border: '1px solid rgba(0,229,255,0.15)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  ...
                  {/* Login Email — max 100 chars, must contain @ */}
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                <motion.button className="btn btn-primary" style={{ flex: 1, gap: '0.4rem' }} whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }} onClick={handleRegister}>
                ...
                : <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {history.map(r => (
                      <div key={r.id} style={{ padding: '1rem', borderRadius: 12, background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-1)', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span className="mono" style={{ fontSize: '0.82rem', color: 'var(--primary)', fontWeight: 700 }}>{r.id}</span>
