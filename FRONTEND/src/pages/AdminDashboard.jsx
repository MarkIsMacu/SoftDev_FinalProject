...
import {
  ...
  Activity, ArrowUpRight, X, Download, FileText, RefreshCw
} from 'lucide-react';
...
const AdminDashboard = () => {
  const { tickets, customers, activity, getStats, refreshStats, refreshTickets } = useData();
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    await Promise.all([refreshStats(), refreshTickets()]).catch(() => { });
    setRefreshing(false);
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `comprepair_tickets_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  const recentActivity = (activity.length > 0 ? activity : []).slice(0, 8);
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.15rem', gap: '0.5rem' }}>
                      <span className="mono" style={{ fontSize: '0.75rem', color: logColor(entry.action), whiteSpace: 'nowrap' }}>
                        {entry.ticketId}
                      </span>
