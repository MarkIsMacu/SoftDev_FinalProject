/**
 * generatePDFReport — Opens a formatted, print-ready report in a new window.
 * The user can save it as PDF via the browser's Print → Save as PDF dialog.
 * No external libraries needed; works in all modern browsers.
 *
 * Backend integration point:
 *   When backend is live, replace with an API call that returns a PDF blob:
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ── Header ── */
    .header {
      background: linear-gradient(135deg, #0a0a1a 0%, #001133 100%);
      color: #fff;
    ...
    .header::after {
      ...
      background: linear-gradient(90deg, #00e5ff, #0066ff, #b06aff);
    }
    ...
    /* ── Summary box ── */
    .summary-box {
      border: 1px solid #e0e0ee; border-radius: 10px; padding: 1.5rem 2rem;
      background: #f8f8fc; margin-bottom: 2rem;
    ...
    .footer {
      ...
      background: #f8f8fc; border-top: 1px solid #e8e8f0;
    ...
    .print-btn {
      ...
      padding: 0.55rem 1.5rem; font-family: 'Space Grotesk', sans-serif;
...
<body>
    ...
    <div class="header-top">
      <div>
        ...
        <div class="report-subtitle">
          Period: ${fromDate ? fromDate : 'All time'} — ${toDate ? toDate : 'Present'}
        </div>
      </div>
  ...
  <div class="body">
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card">
        ...
        <div class="kpi-value pink">${waiting}</div>
      </div>
    </div>
    ...
    <div class="summary-box">
      <div>
        <div class="summary-row"><span class="summary-key">Tickets Received</span><span class="summary-val">${received}</span></div>
    ...
    <table>
      <thead>
        <tr><th>Client ID</th><th>Name</th><th>Contact</th><th>Total Repairs</th><th>Joined</th></tr>
      </thead>
      <tbody>
        ${customers.map(c => `
          <tr>
            ...
            <td>${c.email}</td>
            <td style="text-align:center;font-weight:700">${c.totalRepairs}</td>
            <td>${c.joined}</td>
          </tr>`).join('')}
      </tbody>
    </table>
  ...
  <script>
    // Auto-open print dialog after page loads for a seamless "save PDF" flow
    // Comment out if you prefer the user to press the button manually
